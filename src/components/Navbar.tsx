...
export function Navbar() {
  ...
  useEffect(() => { setMobileOpen(false); setSearchOpen(false); setUserMenu(false); setShopDrop(false) }, [pathname])
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center h-16 gap-2">
          ...
          <Link href="/" className="flex items-center gap-2 flex-shrink-0 group mr-4">
            <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-cx-emerald to-cx-sky flex items-center justify-center shadow-[0_0_16px_rgba(16,217,136,0.3)] group-hover:shadow-[0_0_28px_rgba(16,217,136,0.55)] transition-all duration-300">
          ...
          <nav className="hidden lg:flex items-center gap-1 flex-1 justify-center">
            {NAV_LINKS.map(link => (
              <div key={link.label} className="relative"
                onMouseEnter={() => link.label === 'Shop' && setShopDrop(true)}
                onMouseLeave={() => setShopDrop(false)}>
                <Link href={link.href}
                  className={cn('flex items-center gap-1 px-3 py-2 rounded-xl text-[13px] font-600 transition-all duration-200 relative',
                    isActive(link.href) ? 'text-cx-emerald nav-active' : 'text-cx-dim hover:text-cx-text hover:bg-white/4')}>
                  {link.label}
                  {link.label === 'Shop' && <ChevronDown size={12} className={cn('transition-transform duration-200 ml-0.5', shopDrop && 'rotate-180')} />}
                </Link>

                {/* Shop mega dropdown */}
                {link.label === 'Shop' && shopDrop && (
                  <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1 w-72 glass-dark rounded-2xl border border-cx-border shadow-[0_20px_60px_rgba(0,0,0,0.6)] overflow-hidden animate-slide-down z-50">
                    <div className="p-3">
                      <Link href="/products" className="flex items-center gap-2 px-3 py-2.5 rounded-xl text-[13px] font-700 text-cx-emerald hover:bg-cx-emerald/8 transition-colors mb-1">
                        <Zap size={14}/> All Products
                      </Link>
                      <div className="section-divider mb-2" />
                      <div className="grid grid-cols-2 gap-1">
                        {CATEGORIES.map(cat => (
                          <Link key={cat.label} href={cat.href}
                            className="flex items-center gap-2 px-3 py-2 rounded-xl text-[12px] font-600 text-cx-dim hover:text-cx-text hover:bg-white/4 transition-colors">
          ...
          <div className="flex items-center gap-1 ml-auto">
            ...
            <ClientOnly>
              <Link href="/wishlist" className="relative p-2 rounded-xl text-cx-muted hover:text-cx-rose hover:bg-cx-rose/10 transition-all">
                <Heart size={18}/>
                {wishCount > 0 && <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-cx-rose text-white text-[9px] font-700 rounded-full flex items-center justify-center notif-dot">{wishCount > 9 ? '9+' : wishCount}</span>}
              </Link>
              <button onClick={toggleCart} className="relative p-2 rounded-xl text-cx-muted hover:text-cx-emerald hover:bg-cx-emerald/10 transition-all">
                ...
                {totalItems > 0 && <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-gradient-to-br from-cx-emerald to-cx-sky text-cx-bg text-[9px] font-700 rounded-full flex items-center justify-center animate-glow-pulse">{totalItems > 9 ? '9+' : totalItems}</span>}
              </button>
            </ClientOnly>
                  ...
                  {userMenu && (
                    <div className="absolute top-full right-0 mt-2 w-52 glass-dark rounded-2xl border border-cx-border shadow-[0_20px_60px_rgba(0,0,0,0.6)] overflow-hidden animate-scale-in z-50">
                      <div className="px-4 py-3 border-b border-cx-border">
                        <p className="font-600 text-[13px] text-cx-text">{session.user?.name}</p>
          ...
          <div className="lg:hidden pb-4 border-t border-cx-border/40 pt-3 animate-slide-down space-y-0.5">
            {NAV_LINKS.map(link => (
              <Link key={link.label} href={link.href}
                className={cn('block px-3 py-2.5 rounded-xl text-[13px] font-600 transition-colors', isActive(link.href) ? 'text-cx-emerald bg-cx-emerald/5' : 'text-cx-dim hover:text-cx-text hover:bg-white/3')}>
                {link.label}
              </Link>
            ))}
