...
export function ContactForm() {
  ...
  const validate = () => {
    ...
    if (!f.email.match(/^[^\s@]+@[^\s@]+\.[^\s@]+$/)) return 'Please enter a valid email address'
  ...
  if (status === 'success') return (
    <div className="p-8 rounded-3xl cx-card-flat flex flex-col items-center gap-5 min-h-72 justify-center text-center">
  ...
  return (
    <form onSubmit={submit} className="p-6 sm:p-8 rounded-3xl cx-card-flat">
      ...
      <p className="text-[12px] text-cx-muted mb-6">Enter your email address so we can reply to you directly. All messages are delivered securely to our team.</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
        ...
        <div>
          <label className="block text-[11px] font-700 text-cx-muted uppercase tracking-wider mb-2">
            Your Email * <span className="text-cx-emerald normal-case font-400">(for reply)</span>
          </label>
          <div className="relative">
        ...
        <div>
          ...
          <div className="relative">
            ...
            <select value={f.category} onChange={e => set('category', e.target.value)}
              className="cx-input w-full pl-9 pr-4 py-3 text-[13px] appearance-none">
              {['support','partnership','press','general'].map(c => (
      ...
      <div className="mb-4">
        ...
        <div className="relative">
          ...
          <input value={f.subject} onChange={e => set('subject', e.target.value)} placeholder="Brief description of your enquiry"
            className="cx-input w-full pl-9 pr-4 py-3 text-[13px]"/>
        </div>
      </div>
      ...
      <div className="mb-6">
        ...
        <textarea value={f.message} onChange={e => set('message', e.target.value)}
          ...
          className="cx-input w-full px-4 py-3 text-[13px] resize-y min-h-[120px]"/>
        <p className="text-[11px] text-cx-muted mt-1.5">Your message will be securely delivered to our team. We'll reply to your email address.</p>
      </div>
      ...
      <button type="submit" disabled={status === 'loading'}
        ...
        {status === 'loading'
          ? <><Loader2 size={15} className="animate-spin"/> Sending securely…</>
