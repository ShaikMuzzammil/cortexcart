...
import { Star, Shield, Truck, RotateCcw, ChevronRight, Award, Tag } from 'lucide-react'

async function getProduct(slug: string) {
  return prisma.product.findUnique({
    where: { slug },
    include: { category:true, reviews:{ include:{ user:{ select:{ name:true } } }, orderBy:{ createdAt:'desc' }, take:8 } },
  })
}
...
export default async function ProductPage({ params }: { params:{ slug:string } }) {
  ...
  const related = await prisma.product.findMany({ where:{ categoryId:product.categoryId, id:{ not:product.id }, isActive:true }, take:4 })
    ...
    <div className="min-h-screen pt-6 pb-20 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <nav className="flex items-center gap-2 text-[11px] text-cx-muted mb-8 flex-wrap">
          <a href="/" className="hover:text-cx-emerald transition-colors">Home</a><ChevronRight size={11}/>
          <a href="/products" className="hover:text-cx-emerald transition-colors">Products</a><ChevronRight size={11}/>
          <a href={`/products?category=${product.category.slug}`} className="hover:text-cx-emerald transition-colors">{product.category.name}</a><ChevronRight size={11}/>
          <span className="text-cx-text truncate max-w-[180px]">{product.name}</span>
        </nav>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-20">
          ...
          <div className="space-y-4">
            <div className="relative aspect-square rounded-3xl overflow-hidden cx-card-flat">
              <Image src={product.images[0]} alt={product.name} fill className="object-cover" sizes="(max-width:1024px)100vw,50vw" priority />
              {disc > 0 && <div className="absolute top-4 left-4 badge-rose text-sm px-3 py-1 rounded-xl font-700">-{disc}% OFF</div>}
            </div>
            {product.images.length > 1 && (
              <div className="grid grid-cols-4 gap-3">
                {product.images.slice(0,4).map((img,i) => (
                  <div key={i} className="relative aspect-square rounded-2xl overflow-hidden cx-card-flat hover:border-cx-emerald/40 transition-colors cursor-pointer">
          ...
          <div className="space-y-5">
            ...
            <h1 className="font-display font-900 text-3xl sm:text-4xl text-white leading-tight">{product.name}</h1>
            ...
            <div className="flex items-end gap-4">
              <span className="font-display font-900 text-4xl grad-emerald num">{formatPrice(product.currentPrice)}</span>
              {product.comparePrice && product.comparePrice > product.currentPrice && (
                <>
                  ...
                  <span className="badge-rose text-[13px] px-3 py-1 rounded-xl font-700 mb-1">Save {formatPrice(product.comparePrice - product.currentPrice)}</span>
                </>
              )}
            </div>
            ...
            {attrs && (
              <div className="p-4 rounded-2xl cx-card-flat">
                <h3 className="text-[11px] font-700 text-cx-muted uppercase tracking-wider mb-3">Key Specs</h3>
            ...
            {product.tags.length > 0 && (
              <div className="flex items-center gap-2 flex-wrap">
                <Tag size={13} className="text-cx-muted" />
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <div key={t.label} className="flex flex-col items-center gap-1.5 p-3 rounded-2xl cx-card-flat text-center">
                  ...
                  <p className="text-[10px] font-600 text-cx-text">{t.label}</p>
                ...
                <div key={r.id} className="p-5 rounded-2xl cx-card-flat">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      ...
                      {r.verified && <span className="text-[10px] text-cx-emerald">✓ Verified</span>}
                    </div>
        ...
        {related.length > 0 && (
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
              {related.map(p => <ProductCard key={p.id} id={p.id} slug={p.slug} name={p.name} brand={p.brand} price={p.currentPrice} originalPrice={p.basePrice} comparePrice={p.comparePrice} image={p.images[0]} images={p.images} rating={p.rating} reviewCount={p.reviewCount} stock={p.stock} isFeatured={p.isFeatured} />)}
            </div>
          </section>
        )}
      </div>
