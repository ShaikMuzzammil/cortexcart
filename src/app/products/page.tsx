...
async function getProducts(p: SP) {
  ...
  const ob: any = p.sort==='price_asc'?{currentPrice:'asc'}:p.sort==='price_desc'?{currentPrice:'desc'}:p.sort==='newest'?{createdAt:'desc'}:p.sort==='rating'?{rating:'desc'}:{reviewCount:'desc'}
...
export default async function ProductsPage({ searchParams }: { searchParams: SP }) {
  ...
  const title = searchParams.q ? `Results for "${searchParams.q}"` :
    ...
    searchParams.deals==='true'    ? 'Deals & Offers' : 'All Products'
    ...
    <div className="min-h-screen pt-6 pb-20 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-cx-muted text-sm">{products.length} product{products.length!==1?'s':''} found</p>
        </div>
          ...
          <main className="flex-1">
            ...
            {products.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
                ...
                <p className="font-600 text-cx-text text-xl">No products found</p>
                <p className="text-cx-muted text-sm">Try adjusting your filters or search terms</p>
              </div>
            ) : (
