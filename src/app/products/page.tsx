...
export default async function ProductsPage({ searchParams }: { searchParams: SP }) {
  ...
  const title =
    ...
    searchParams.category       ? (activeCategory?.name ?? 'Products') :
    ...
    <div className="min-h-screen pt-6 pb-24 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-cx-muted text-[13px]">
            {products.length} product{products.length !== 1 ? 's' : ''} found
            {searchParams.category && activeCategory && (
              <> in <span className="text-cx-emerald">{activeCategory.name}</span></>
            )}
          </p>
        </div>
            ...
            {products.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-24 gap-5 text-center">
                <div className="w-24 h-24 rounded-3xl cx-card-flat flex items-center justify-center">
                  <Search size={36} className="text-cx-muted" />
                </div>
                <div>
                  <p className="font-display font-700 text-xl text-white mb-2">No products found</p>
                  <p className="text-cx-muted text-[14px] mb-6">
                    {searchParams.q
                      ? `No results for "${searchParams.q}". Try a different search term.`
                      : 'No products match your current filters. Try clearing some filters.'}
                  </p>
                </div>
                <div className="flex flex-wrap gap-3 justify-center">
                  <Link href="/products" className="btn-em px-6 py-2.5 text-[13px] font-700 rounded-xl flex items-center gap-2">
                    <Package size={14} /> Browse All Products
                  </Link>
                  {(searchParams.category || searchParams.brand || searchParams.minPrice) && (
                    <Link href="/products" className="btn-outline-em px-6 py-2.5 text-[13px] rounded-xl">
                      Clear Filters
                    </Link>
                  )}
                </div>
                {/* Show some products anyway */}
                <div className="w-full mt-8">
                  <p className="text-[13px] text-cx-muted mb-4 text-center">You might also like:</p>
