...
import { ShoppingBag, CreditCard, Truck, CheckCircle2, ArrowLeft, ArrowRight, Loader2, Shield, Lock, Zap, BadgeCheck } from 'lucide-react'
...
const CARD_BRANDS = [
  { name:'Visa',       img:'https://upload.wikimedia.org/wikipedia/commons/thumb/5/5e/Visa_Inc._logo.svg/200px-Visa_Inc._logo.svg.png' },
  ...
  { name:'Amex',       img:'https://upload.wikimedia.org/wikipedia/commons/thumb/f/fa/American_Express_logo_%282018%29.svg/200px-American_Express_logo_%282018%29.svg.png' },
]
          ...
          <div className="lg:col-span-2">
            ...
            {step === 2 && (
              <div className="p-6 rounded-3xl cx-card-flat animate-fade-in">
                ...
                <div className="space-y-2 mb-6">
                  {[
                    { id:'standard', label:'Standard Shipping', sub:'3–7 business days',  price: sub>=99 ? 'FREE' : '$9.99' },
                    { id:'express',  label:'Express Shipping',  sub:'1–2 business days',  price: sub>=99 ? 'FREE' : '$19.99' },
                  ].map(opt => (
            ...
            {step === 3 && (
              <div className="p-6 rounded-3xl cx-card-flat animate-fade-in">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="font-display font-700 text-xl text-white">Payment Details</h2>
                ...
                <div className="flex items-center gap-3 mb-5">
                  {CARD_BRANDS.map(b => (
                    <div key={b.name} className="px-3 py-2 rounded-lg cx-card-flat flex items-center justify-center w-14 h-9 opacity-70 hover:opacity-100 transition-opacity">
                      <img src={b.img} alt={b.name} className="max-h-5 max-w-full object-contain filter brightness-90"/>
                    </div>
                  ))}
                ...
                <div className="p-3 rounded-xl bg-cx-emerald/5 border border-cx-emerald/15 mb-5 flex items-center gap-2 text-[11px] text-cx-muted">
                  <BadgeCheck size={14} className="text-cx-emerald flex-shrink-0"/>
                  Your payment information is encrypted with 256-bit SSL. We never store card data.
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={placeOrder} disabled={processing}
                    ...
                    {processing
                      ...
                      : <><Shield size={15}/> Place Order — {formatPrice(total)}</>}
                  </button>
                </div>
              </div>
            )}
            ...
            {step === 4 && orderNum && (
              <div className="p-8 rounded-3xl cx-card-flat text-center animate-fade-in">
                ...
                <p className="text-cx-muted mb-2 text-[14px]">Order <strong className="text-cx-emerald font-mono text-[15px]">#{orderNum}</strong></p>
                <p className="text-[13px] text-cx-muted mb-8 leading-relaxed max-w-sm mx-auto">
                  A confirmation email has been sent to <strong className="text-cx-text">{addr.email}</strong>. Our team has been notified.
                </p>
